...
# Define the old function pattern (from the comment to the closing brace)
old_pattern = r'''    // Bayesian Dose-Response Meta-Analysis using MCMC
...
if match:
    print("Found the fitBayesianModel function!")
    print(f"Old function length: {len(match.group(0))} characters")
    ...
    print("Successfully replaced fitBayesianModel function!")
    print(f"New function length: {len(new_function)} characters")
else:
    print("ERROR: Could not find the fitBayesianModel function!")
    print("Trying to locate it...")
    if 'function fitBayesianModel' in content:
        print("Found 'function fitBayesianModel' in content")
        idx = content.find('function fitBayesianModel')
        print(f"Context around it:\n{content[max(0, idx-100):idx+500]}")
